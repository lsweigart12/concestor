/**
 * PBDB's uncertainty model, drawn as it is recorded.
 *
 ...
 * wearing an observation's clothes. Neither is available here: the geometry
 * below has no code path that produces a single-point summary.
 *
...
export type Bracket =
  ...
  | {
      ...
      certain: Span | null;
      certainty: Certainty;
      /** Oldest and youngest bound in Ma, for the tooltip. */
      oldest: number;
      youngest: number;
      /**
 ...
 */
export function bracketGeom(a: Appearance, toX: (ma: number) => number): Bracket {
 ...
 *
 * The distinction between an empty certain extent and a real one has to
 * survive without the key — it is the difference between "we know it was here"
 * and "we know it was somewhere in here" — so every row carries it in full on
 * hover, not only as a difference in ink.
 */
export function bracketTitle(name: string, b: Bracket): string {
  if (b.kind === "absent") {
 ...
 * "certainly present" for them to look for. The words are the key's whole job;
 * the sentence-length version is on each row's own tooltip.
 */
export function bracketKey(brackets: readonly Bracket[]): BracketKeyRow[] {
